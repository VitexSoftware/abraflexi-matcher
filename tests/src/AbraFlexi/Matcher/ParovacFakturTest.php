...
use \AbraFlexi\Matcher\ParovacFaktur;
...
class ParovacFakturTest extends \Test\Ease\SandTest
{
    ...
    {
        $faktura = $this->makeInvoice(['typDokl' => \AbraFlexi\Functions::code('FAKTURA'),
            'popis' => 'InvoicesMatchingByBank AbraFlexi-Matcher Test']);
        $zaloha = $this->makeInvoice(['typDokl' => \AbraFlexi\Functions::code('ZÁLOHA'),
            'popis' => 'InvoicesMatchingByBank AbraFlexi-Matcher Test']);
        $dobropis = $this->makeInvoice(['typDokl' => \AbraFlexi\Functions::code('DOBROPIS'),
            'popis' => 'InvoicesMatchingByBank AbraFlexi-Matcher Test']);
        $this->object->setStartDay(-1);
        ...
        foreach ($paymentsToCheck as $paymentID => $paymentData) {
            $paymentChecker->loadFromAbraFlexi(\AbraFlexi\Functions::code($paymentData['kod']));
            $this->assertEquals(
                'true',
    ...
    {
        $dobropis = $this->makeInvoice(['typDokl' => \AbraFlexi\Functions::code('ODD'),
    ...
    {
        $this->assertEquals(
            '<a href="'.\Ease\Shared::cfg('ABRAFLEXI_URL').'/c/'.\Ease\Shared::cfg('ABRAFLEXI_COMPANY').'/banka.json" target="_blank" rel="nofollow">https://demo.flexibee.eu:5434/c/demo_de/banka.json</a>',
