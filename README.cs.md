...
Páruje bankovní platby v AbraFlexi s vydanými/přijatými fakturami - podle variabilního symbolu, specifického symbolu, čísla bankovního účtu/IBAN, nebo všechny způsoby najednou - jako samostatné CLI skripty, cron úlohy, nebo aplikace [MultiFlexi](https://multiflexi.eu).

## Obsah

- [Jak to funguje](#jak-to-funguje)
- [Instalace (Debian/Ubuntu)](#instalace-debianubuntu)
- [Příkazy](#příkazy)
- [Konfigurace](#konfigurace)
- [Jazyky](#jazyky)
- [MultiFlexi](#multiflexi)
- [Vývojové diagramy](#vývojové-diagramy)
- [Návratové kódy](#návratové-kódy-exit-codes)
- [Ošetření chyb](#ošetření-chyb)
- [Závislosti](#závislosti)
- [Testování](#testování)
- [Další software pro AbraFlexi](#další-software-pro-abraflexi)
- [Poděkování](#poděkování)

## Jak to funguje

Ke každé nespárované příchozí platbě se program pokusí nalézt vhodný (neuhrazený a nestornovaný) doklad ke spárování - podle variabilního symbolu, specifického symbolu, nebo čísla bankovního účtu/IBAN plátce, podle toho, který skript se spustí. Výsledky jsou sjednoceny podle čísla bankovního pohybu, aby nedocházelo k duplicitám, když faktura vyhoví více různým hledáním.

* Platby, které nemají dohledaný protějšek podle žádné z podmínek, jsou označeny štítkem **CHYBIFAKTURA** (nebo nahlášeny jako `unmatched`).
* Platby, které se vůbec nepodařilo identifikovat, jsou označeny štítkem **NEIDENTIFIKOVANO**.
* Číslo účtu/IBAN přiřazené v adresáři AbraFlexi více firmám je nahlášeno jako `duplicate_buc` místo toho, aby se odhadovalo, ke které firmě patří.
...
* **ZÁLOHA** - zálohová faktura je spárována s platbou + je vytvořen daňový doklad se stejným variabilním symbolem, od kterého je tato záloha odečtena.
* **DOBROPIS** - je proveden odpočet dobropisu.
* Ostatní - je zapsáno varování do protokolu spolu s odkazem do webového AbraFlexi.
...
.deb balíčky jsou k dispozici ze dvou repozitářů, podle toho, zda potřebujete nejnovější vývoj, nebo stabilní produkční verzi:
...
Po instalaci balíčku jednou pro každou firmu v AbraFlexi spusťte **[abraflexi-matcher-init](debian/abraflexi-matcher-init.1)** (vytvoří štítky/typy dokladů, na kterých ostatní skripty závisí).
...
* **[abraflexi-pull-bank](debian/abraflexi-pull-bank.1)** - pouze stáhne bankovní výpisy.
...
* **[abraflexi-match-accountno](debian/abraflexi-match-accountno.1)** - páruje vydané faktury s přijatými platbami pouze podle čísla bankovního účtu/IBAN plátce.
...
Párovač AbraFlexi je připraven běžet jako aplikace [MultiFlexi](https://multiflexi.eu).
Úplný seznam připravených aplikací pro platformu MultiFlexi najdete na [stránce se seznamem aplikací](https://www.multiflexi.eu/apps.php).
...
Tento software by nevznikl bez podpory:
